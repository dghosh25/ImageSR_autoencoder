...
def psnr_metric(y_true, y_pred):
    # Assuming the pixel values are scaled between 0 and 1
    return psnr(y_true, y_pred, max_val=1.0)

# Load the autoencoder model
autoencoder_bilinear = tf.keras.models.load_model('autoencoder_x2_psnr.h5')
...
for subdir in subdirs:
    ...
    for filename in os.listdir(path):
        if filename.endswith('LR.png'):
            ...
            sr_filename = filename.replace('LR.png', 'SR_psnr.png')
